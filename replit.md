...
## Recent Changes
- **2026-02-23 (latest)**: Fixed LLM argument parsing issues:
  - `run_shell`: Auto-strips prefixes ("shell:", "bash:", "command:" dll) dari command sebelum eksekusi
  - `install_package`: Auto-strips prefixes dan noise words, package name extraction lebih akurat
  - `create_directory`: Regex diperbaiki untuk menangani "bernama/named/called" pattern
  - `apply_patch`: System prompt ditambah instruksi lebih kuat agar LLM selalu call tool
- **2026-02-23**: Fixed publish/deploy error - root `/` endpoint no longer blocks on provider loading (returns instantly). Removed `--preload` flag from Gunicorn to prevent DB connection issues after fork. Provider data loads in background thread.
- **2026-02-23**: Tested all 11 user-facing agent tools via `/v1/agent/completions` - semua OK
